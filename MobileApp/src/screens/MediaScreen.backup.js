...
const MediaScreen = () => {
  ...
  // Add this helper function to ensure permissions
  ...
  const handleRemoteVideoStop = async (commandId) => {
    try {
      if (MediaCaptureService.isVideoRecording()) {
        ...
        Alert.alert('Remote Recording Stopped', 'Video recording stopped by remote command!');
  ...
  const takePhoto = async () => {
    ...
    try {
      ...
      if (cameraRef.current) {
        MediaCaptureService.setCameraRef(cameraRef.current);
        await MediaCaptureService.capturePhoto();
  ...
  const recordVideo = async () => {
    ...
    try {
      if (isRecording) {
        ...
        if (cameraRef.current) {
          MediaCaptureService.setCameraRef(cameraRef.current);
          await MediaCaptureService.startVideoRecording({ maxDuration: 30 });
        }
      }
    } catch (error) {
          ...
          <View style={styles.cameraOverlay}>
            {remoteCommandActive && (
              <View style={styles.remoteIndicator}>
                <Ionicons name="radio" size={20} color="#FF6B6B" />
                <Text style={styles.remoteText}>
                  Remote {commandType === 'capture-photo' ? 'Photo' : 'Video'} Command
                </Text>
              </View>
            )}
      ...
      <View style={styles.featuresContainer}>
        ...
        <View style={styles.featureCard}>
          ...
          <View style={styles.featureText}>
            ...
            <Text style={styles.featureDescription}>
              Start/stop video recording from dashboard
            </Text>
          </View>
        </View>
        ...
        <View style={styles.featureCard}>
          ...
          <View style={styles.featureText}>
            <Text style={styles.featureTitle}>Auto Upload & Save</Text>
            <Text style={styles.featureDescription}>
              Media saved to gallery and uploaded to cloud
            </Text>
          </View>
        </View>
      </View>
      ...
      {!hasPermissions && (
        <View style={styles.permissionContainer}>
          ...
          <Text style={styles.permissionText}>
            Camera and media library permissions are required for remote capture functionality
          </Text>
          <TouchableOpacity
            style={styles.permissionButton}
            onPress={initializeMediaService}
          >
